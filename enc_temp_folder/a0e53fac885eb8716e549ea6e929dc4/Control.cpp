...
void Control::menuPrincipal() {
	...
	do {
		...
		case 2: {
			...
			int numSuc;
			while (true) {
				cout << endl << "Ingrese el numero de la sucursal a eliminar (o 'n' para cancelar): ";
				cin >> nSuc;
				cin.ignore(10000, '\n');
				if (nSuc == "n") {
				...
				if (numSuc < 0 || numSuc >= numeroSucursales) {
					cout << "Numero de sucursal invalido. Intente de nuevo." << endl;
					continue;
				}
				char conf;
				while (true) {
					cout << "Esta seguro que desea eliminar la sucursal #" << numSuc << "? (s/n): ";
					cin >> conf;
					cin.ignore(10000, '\n');

					if (conf == 's' || conf == 'S') {
						if (eliminarSucursal(numSuc)) {
							cout << endl << "Sucursal #" << numSuc << " eliminada exitosamente." << endl;
						}
						else {
							cout << endl << "Error: No se pudo eliminar la sucursal #" << numSuc << "." << endl;
						}
						cout << endl << "Aprete enter para volver al menu principal" << endl;
						cin.get();
						break;
...
void Control::subMenuColaboradores(Sucursal* s) {
	...
	do {
		...
		case 3: {
			...
			funcionMostrarColaboradoresDeLaSucursal(s);
			cout << "Aprete enter para volver al submenu de colaboradores" << endl;
			cin.clear();
			cin.get();
			break;
		...
		case 4: {
			...
			funcionReporteAlquiContraPorColaborador(s);
			cout << "Aprete enter para volver al submenu de colaboradores" << endl;
			cin.get();
			break;
		}
...
void Control::subMenuClientes(Sucursal* s) {
	...
	do {
		...
		case 3: {
			...
			funcionMostrarCLienteDeLaSucursal(s);
		...
		case 4: {
			...
			funcionHistorialDeSoliContraPorCliente(s);
			cout << endl << "Aprete enter para volver al submenu de clientes" << endl;
			cin.get();
			break;
		}
		case 5: {
			...
			funcionReporteDeClientePorCantidadContratos(s);
			cout << endl << "Aprete enter para volver al submenu de clientes" << endl;
			cin.get();
			break;
		}
...
void Control::funcionAgregarColaborador(Sucursal* s) {
	if (!s) return;
	system("cls");
	bool seguir = true;
	while (seguir) {
...
void Control::funcionEliminarColaborador(Sucursal* s) {
	system("cls");
	bool seguir = true;
	while (seguir) {
		string cedula;
		...
		if (conf == 'n' || conf == 'N') {
			...
			while (true) {
				...
				cin.ignore(10000, '\n');
				if (op == 's' || op == 'S') break;
				if (op == 'n' || op == 'N') { seguir = false; return; }
				cout << "Opcion invalida. Intente de nuevo." << endl;
			}
			continue;
		}
...
void Control::funcionMostrarColaboradoresDeLaSucursal(Sucursal* s) {
	if (!s) return;
	system("cls");
	cout << " Colaboradores de la sucursal " << s->getNumSucursal() << endl << endl;
	s->getColaboradores()->mostrarColaboradores();
}
void Control::funcionReporteAlquiContraPorColaborador(Sucursal* s) {
	bool seguir = true;
	while (seguir) {
		system("cls");
		string cedula;
		...
		while (true) {
			...
			if (opcionDetalle == 's' || opcionDetalle == 'S') {
				string codigo;
				while (true) {
					cout << "Ingrese el codigo de la solicitud o contrato (o 'n' para cancelar): ";
					cin >> codigo;
					cin.ignore(10000, '\n');
					if (codigo == "n" || codigo == "N") break;
					if (codigo.empty()) {
						cout << "El codigo no puede estar vacio. Intente de nuevo." << endl;
						continue;
					}
		...
		while (true) {
			...
			if (opcionOtro == 'n' || opcionOtro == 'N') { seguir = false; return; }
			cout << "Opcion invalida. Intente de nuevo." << endl;
		}
	}
}
void Control::funcionAgregarCliente(Sucursal* s) {
	bool seguir = true;
	while (seguir) {
...
void Control::funcionEliminarCliente(Sucursal* s) {
	bool seguir = true;
	while (seguir) {
		system("cls");
		string cedula;
...
void Control::funcionMostrarCLienteDeLaSucursal(Sucursal* s) {
	if (!s) return;
	system("cls");
	cout << " Clientes de la sucursal " << s->getNumSucursal() << endl << endl;
	s->getClientes()->mostrarClientes();
}
void Control::funcionHistorialDeSoliContraPorCliente(Sucursal* s) {
	if (!s) return;
	bool seguir = true;
	while (seguir) {
		system("cls");
...
void Control::funcionReporteDeClientePorCantidadContratos(Sucursal* s) {
	if (!s) return;
	system("cls");
